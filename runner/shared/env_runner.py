...
class EnvRunner(Runner):
    ...
    def __init__(self, config):
        ...
        #
        # # 缓冲区配置
        # self.buffer_flush_interval = 500  # 每50个episode写入一次
        ...
        """  
        10.27 把1000个episode的结果保存到文件里     
        """
        # 从base中读取USV数量
        self.num_usv = self.base.num_usv
    #   把结果保存在metrics_new文件夹下面，然后这个文件夹下面，存在的是以usv_num为前缀的子文件夹，每个子文件夹下保存的是每个episode的数据，其中以时间戳为后缀，并且保存为txt文件
        """10.27 """
        # 创建 metrics_new 目录结构
        self.metrics_dir = str(self.run_dir / 'metrics_new')
        if not os.path.exists(self.metrics_dir):
            os.makedirs(self.metrics_dir)

        # 创建以 usv_num 为前缀的子目录
        self.usv_metrics_dir = os.path.join(self.metrics_dir, f'usv_{self.num_usv}')
        if not os.path.exists(self.usv_metrics_dir):
            os.makedirs(self.usv_metrics_dir)
        # ==== 新增代码结束 ====
    ...
    """ 10.27"""
    def _save_metrics_to_txt(self, metrics_data):
        """将episode级别的指标数据保存到纯文本文件"""
        timestamp = time.strftime("%Y%m%d_%H%M%S")

        # 定义要保存的指标及其对应的数据
        metrics_to_save = {
            'epi_rewards': metrics_data['rewards'],
            'epi_system_time': metrics_data['system_time'],
            'epi_total_energy': metrics_data['total_energy'],
            'epi_uav_energy': metrics_data['uav_energy'],
            'epi_usv_energy': metrics_data['usv_energy'],
            'epi_completion_rate': metrics_data['completion_rate'],
            'epi_uav_fly_energy': metrics_data['uav_fly_energy'],
            'epi_uav_comp_energy': metrics_data['uav_comp_energy']
        }

        # 保存每个指标到单独的文件
        for metric_name, data in metrics_to_save.items():
            filename = f"usv{self.num_usv}_{metric_name}.txt"
            filepath = os.path.join(self.usv_metrics_dir, filename)
            ...
            print(f"Saved {len(data)} episode {metric_name} values to {filepath}")
    ...
    def run(self):
        ...
        """
        10.27 注释掉了磁盘保存，修改成txt文件存储
        """
        # 首先声明info中的信息，以episode的形式保存，episode以变量保存
        epi_rewards = []
        epi_system_time = []
        epi_total_energy = []
        epi_uav_energy = []
        ...
        for episode in range(episodes):
            ...
            """
            10.27 在episode外面声明几个变量，用于存储60个step的结果，最后求平均值以后存储在epi[]里面
            """
            step_rewards = 0
            step_system_time = 0
            step_total_energy = 0
            ...
            step_uav_fly_energy = 0
            step_uav_comp_energy = 0
            ...
            for step in range(self.episode_length):
                ...
                print(info)
                """
                10.27 把info字典信息存储在之前那几个对应的step变量里面
                """
                step_rewards += np.mean(rewards)
                # print( "rewards:",rewards)
                step_system_time += info.get('system_time', 0)
                step_total_energy += info.get('total_energy', 0)
                step_uav_energy += info.get('avg_uav_energy', 0)
                step_usv_energy += info.get('avg_usv_energy', 0)
                step_completion_rate += info.get('completion_rate', 0)
                step_uav_fly_energy += info.get('avg_uav_fly_energy', 0)
                step_uav_comp_energy += info.get('avg_uav_comp_energy', 0)
            ...
            self.compute()
            """
            10.27 episode结束以后，把step级别的数据存储到epi级别的数组里面，其中step一共是60，step级别的数据要除以60
            """
            print(self.episode_length)
            epi_rewards.append(step_rewards / self.episode_length)
            # print("epi_rewards:", step_rewards)
            epi_system_time.append(step_system_time / self.episode_length)
            epi_total_energy.append(step_total_energy / self.episode_length)
            epi_uav_energy.append(step_uav_energy / self.episode_length)
            epi_usv_energy.append(step_usv_energy / self.episode_length)
            epi_completion_rate.append(step_completion_rate / self.episode_length)
            epi_uav_fly_energy.append(step_uav_fly_energy / self.episode_length)
            # print("epi_uav_fly_energy:", step_uav_fly_energy)
            epi_uav_comp_energy.append(step_uav_comp_energy / self.episode_length)
        ...
        metrics_data = {
            'rewards': epi_rewards,
            'system_time': epi_system_time,
            'total_energy': epi_total_energy,
            'uav_energy': epi_uav_energy,
            'usv_energy': epi_usv_energy,
            'completion_rate': epi_completion_rate,
            'uav_fly_energy': epi_uav_fly_energy,
            'uav_comp_energy': epi_uav_comp_energy
        }
        self._save_metrics_to_txt(metrics_data)
        # # ========== 训练结束后处理 ==========
        # # 1. 写入缓冲区剩余数据
        # self.flush_metrics_to_disk()
